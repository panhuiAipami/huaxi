...
public class PagerIndicator extends FrameLayout {
    ...
    private android.support.v4.view.ViewPager.OnPageChangeListener delegateListener;
    ...
    private void initAttrs(AttributeSet attrs) {
        textColorId = github.chenupt.springindicator.R.color.si_default_text_color;
        selectedTextColorId = github.chenupt.springindicator.R.color.si_default_text_color_selected;
        indicatorColorId = github.chenupt.springindicator.R.color.si_default_indicator_bg;
        textSize = getResources().getDimension(github.chenupt.springindicator.R.dimen
                .si_default_text_size);
        radiusMax = getResources().getDimension(github.chenupt.springindicator.R.dimen
                .si_default_radius_max);
        radiusMin = getResources().getDimension(github.chenupt.springindicator.R.dimen
                .si_default_radius_min);

        TypedArray a = getContext().obtainStyledAttributes(attrs, github.chenupt.springindicator
                .R.styleable.SpringIndicator);
        textColorId = a.getResourceId(github.chenupt.springindicator.R.styleable
                .SpringIndicator_siTextColor, textColorId);
        selectedTextColorId = a.getResourceId(github.chenupt.springindicator.R.styleable
                .SpringIndicator_siSelectedTextColor, selectedTextColorId);
        textSize = a.getDimension(github.chenupt.springindicator.R.styleable
                .SpringIndicator_siTextSize, textSize);
        textBgResId = a.getResourceId(github.chenupt.springindicator.R.styleable
                .SpringIndicator_siTextBg, 0);
        indicatorColorId = a.getResourceId(github.chenupt.springindicator.R.styleable
                .SpringIndicator_siIndicatorColor, indicatorColorId);
        indicatorColorsId = a.getResourceId(github.chenupt.springindicator.R.styleable
                .SpringIndicator_siIndicatorColors, 0);
        radiusMax = a.getDimension(github.chenupt.springindicator.R.styleable
                .SpringIndicator_siRadiusMax, radiusMax);
        radiusMin = a.getDimension(github.chenupt.springindicator.R.styleable
                .SpringIndicator_siRadiusMin, radiusMin);
        a.recycle();
    ...
    public void setViewPager(final android.support.v4.view.ViewPager viewPager) {
    ...
    public void reSetViewPager(final android.support.v4.view.ViewPager viewPager) {
        if(springView==null){
    ...
    private void addTabContainerView() {
        ...
        tabContainer.setLayoutParams(new LinearLayout.LayoutParams(ViewGroup.LayoutParams
                .MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT, 1.0f));
    ...
    private void addTabItems() {
        ...
        tabs = new ArrayList<RelativeLayout>();
        for (int i = 0; i < viewPager.getAdapter().getCount(); i++) {
            ...
            textView.setLayoutParams(new LinearLayout.LayoutParams((int) (radiusMax * 2), (int)
                    (radiusMax * 2)));
    ...
    private void createPoints() {
        if(viewPager==null || tabs==null || springView==null){
            return;
        }
        if ( viewPager.getCurrentItem() >= tabs.size()) {
            return;
        }
    ...
    private void setUpListener() {
        viewPager.setOnPageChangeListener(new android.support.v4.view.ViewPager
            ...
            public void onPageScrolled(int position, float positionOffset, int
                ...
                if (position < tabs.size() - 1) {
                    ...
                    if (positionOffset < radiusOffsetHead) {
                        ...
                        springView.getHeadPoint().setRadius(((positionOffset - radiusOffsetHead)
                                / (1 - radiusOffsetHead) * radiusOffset + radiusMin));
                    }
                    float radiusOffsetFoot = 0.5f;
                    if (positionOffset < radiusOffsetFoot) {
                        springView.getFootPoint().setRadius((1 - positionOffset /
                                radiusOffsetFoot) * radiusOffset + radiusMin);
                    } else {
                    ...
                    if (positionOffset < headMoveOffset) {
                        ...
                        headX = (float) ((Math.atan(positionOffsetTemp * acceleration * 2 -
                                acceleration) + (Math.atan(acceleration))) / (2 * (Math.atan
                                (acceleration))));
                    }
                    springView.getHeadPoint().setX(getTabX(position) - headX *
                            getPositionDistance(position));
                    float footX = 0f;
                    if (positionOffset > footMoveOffset) {
                        float positionOffsetTemp = (positionOffset - footMoveOffset) / (1 -
                                footMoveOffset);
                        footX = (float) ((Math.atan(positionOffsetTemp * acceleration * 2 -
                                acceleration) + (Math.atan(acceleration))) / (2 * (Math.atan
                                (acceleration))));
                    }
                    springView.getFootPoint().setX(getTabX(position) - footX *
                            getPositionDistance(position));
    ...
    private void setSelectedTextColor(int position) {
        ...
        for (RelativeLayout tab : tabs) {
            ((TextView) tab.getChildAt(0)).setTextColor(getResources().getColor(textColorId));
        }
        if(tabs.size()>position){
            ((TextView) tabs.get(position).getChildAt(0)).setTextColor(getResources().getColor
                    (selectedTextColorId));
        }
    }
