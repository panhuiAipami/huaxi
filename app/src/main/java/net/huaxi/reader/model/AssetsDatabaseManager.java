...
//    }
     ...
     */
    public static AssetsDatabaseManager getManager(Context context) {
        if (mInstance == null) {
            mInstance = new AssetsDatabaseManager(context);
        }
        return mInstance;
    ...
    public SQLiteDatabase getDatabase(String dbfile) {
        ...
        //判断软件版本是否更新，如果跟新，则需要重新重新拷贝数据库，所以把flag=false
        if (dbNeedUpdate()) {
            flag = false;
        }
        if (!flag || !file.exists()) {
    ...
    /**
     * app是否需要从assets中拷贝新DB数据库
     *
     * 更具版本号更新，如果版本更新了，则需要更新数据库
     *
     * @return
     */
    private boolean dbNeedUpdate() {
        ...
        try {
            ...
            SharedPreferences sp = context.getSharedPreferences("SearchVersionCode", 0);
            int spCode = sp.getInt("code", 0);
            if (versionCode == spCode) {
                isNewVersion = false;
            } else {
                sp.edit().putInt("code", versionCode).commit();
            }
