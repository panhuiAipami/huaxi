...
public abstract class DownLoadSecondAdapter<GVH, SVH extends RecyclerView.ViewHolder> extends RecyclerView
        .Adapter<RecyclerView.ViewHolder> {

    private List<Boolean> groupItemStatus = new ArrayList<>();

    private List<GroupChapter> dataTrees = new ArrayList<>();
    ...
    public final void notifyItemC(int p) {
        notifyItemChanged(p);
    }
    ...
    public final void onBindViewHolder(final RecyclerView.ViewHolder holder, int position) {
        ...
        final GroupChapter dt = dataTrees.get(itemStatus.getGroupItemIndex());
            ...
            holder.itemView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (!groupItemStatus.get(groupItemIndex)) {
                        ...
                        notifyItemRangeInserted(holder.getAdapterPosition() + 1, dt.getmChapterList().size());
                        ...
                        notifyItemRangeRemoved(holder.getAdapterPosition() + 1, dt.getmChapterList
                                ().size());
    ...
    public final int getItemCount() {
        ...
        for (int i = 0; i < dataTrees.size(); i++) {
            ...
            if (groupItemStatus.get(i)) {
                itemCount += dataTrees.get(i).getmChapterList().size() + 1;
            } else {
                itemCount++;
            }
    ...
    private ItemStatus getItemStatusByPosition(int position) {
        ...
        for (i = 0; i < groupItemStatus.size(); i++) {
            ...
            if (count == position) {
                ...
                itemStatus.setSubItemIndex(position - (count - dataTrees.get(i - 1).getmChapterList
                        ().size()));
                break;
            ...
            if (groupItemStatus.get(i)) {
                ...
                count += dataTrees.get(i).getmChapterList().size();
        ...
        if (i >= groupItemStatus.size()) {
            itemStatus.setGroupItemIndex(i - 1);
            itemStatus.setViewType(ItemStatus.VIEW_TYPE_SUBITEM);
            itemStatus.setSubItemIndex(position - (count - dataTrees.get(i - 1).getmChapterList().size
                    ()));
        }
