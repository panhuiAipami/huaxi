...
public class OverlappedWidget extends BaseReadView {
    ...
    protected void drawCurrentPageShadow(Canvas canvas) {
        ...
        if (actiondownX > mScreenWidth >> 1) {
            ...
            shadow.setBounds((int) (mScreenWidth + touch_down - DRAW_VALUE), 0, (int) (mScreenWidth + touch_down + DRAW_VALUE), mScreenHeight);

        } else {
            shadow = mBackShadowDrawableRL;
            shadow.setBounds((int) (touch_down - DRAW_VALUE), 0, (int) (touch_down + DRAW_VALUE), mScreenHeight);
        }
        shadow.draw(canvas);
        try {
            canvas.restore();
